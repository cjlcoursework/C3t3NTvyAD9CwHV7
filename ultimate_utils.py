...
def correlate_to_target(df: pd.DataFrame, target_column: str, top_n: int, method='pearson') -> (pd.DataFrame, Set):
...
def plot_heatmap(df, cmap='coolwarm', label_fontsize=10, method='pearson'):  # Add a parameter for x label font size
    numeric_df = df.select_dtypes(include=['number'])
    correlation_matrix = numeric_df.corr(method)
    plt.figure(figsize=(10, 10))
...
def xgboost_predict(X_train, y_train, ):
    ...
    data = pd.DataFrame([
        {'order_delivered': 3, 'order_correct': 3, 'menu_complete': 3, 'price': 4, 'courier': 2, 'app': 4},
        {'order_delivered': 3, 'order_correct': 2, 'menu_complete': 3, 'price': 5, 'courier': 4, 'app': 3},
        {'order_delivered': 5, 'order_correct': 3, 'menu_complete': 3, 'price': 3, 'courier': 3, 'app': 5},
        {'order_delivered': 5, 'order_correct': 4, 'menu_complete': 3, 'price': 3, 'courier': 3, 'app': 5},
        {'order_delivered': 5, 'order_correct': 4, 'menu_complete': 3, 'price': 3, 'courier': 3, 'app': 5}
    ])

    # Assuming you have the target variable in a separate Series
    target = pd.Series([0, 1, 1, 0, 0])  # Replace with your actual target variable
    ...
    # Evaluate the model (for binary classification, you can use accuracy as a metric)
